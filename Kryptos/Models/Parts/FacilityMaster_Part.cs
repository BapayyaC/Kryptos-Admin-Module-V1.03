...
namespace Kryptos.Models
{
    public partial class FacilityMaster
    {
      static   kryptoEntities1 ke = new kryptoEntities1();
        ...
        {
            return ke.UserLoginInformations.Where(x => x.FacilityId == FacilityMasterId).ToList();
        }

        public List<UserLoginInformation> GetAssocaitedActiveUsers()
        {
            return ke.UserLoginInformations.Where(x => x.FacilityId == FacilityMasterId && x.IsActive == true).ToList();
        }

        public List<UserLoginInformation> GetAssocaitedInActiveUsers()
        {
            return ke.UserLoginInformations.Where(x => x.FacilityId == FacilityMasterId && x.IsActive == false).ToList();
        }
        ...
        {
            return ke.UserLoginInformations.Where(x => x.FacilityId == FacilityMasterId && x.IsOrganisationAdmin).ToList();
        }
        ...
        {
            return ke.UserLoginInformations.Where(x => x.FacilityId == FacilityMasterId && x.IsFacilityAdmin && x.IsActive).ToList();
        }
