...
namespace Kryptos.Controllers
{
    public class UserDatatablesController : Controller
    {
        ...
        public bool SendEmail(string subject, string message, string receivermail, string receivername)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    var senderemail = new MailAddress(WebConfigurationManager.AppSettings["SendersEmailAddress"], WebConfigurationManager.AppSettings["SendersName"]);
                    var ps = new MailAddress(receivermail, receivername);
                    var password = WebConfigurationManager.AppSettings["SendersPassword"];
                    var body = message;
                    var sub = subject;
                    var smtp = new SmtpClient
                    {
                        Host = WebConfigurationManager.AppSettings["EmailHost"],
                        Port = int.Parse(WebConfigurationManager.AppSettings["EmailPort"]),
                        EnableSsl = true,
                        DeliveryMethod = SmtpDeliveryMethod.Network,
                        UseDefaultCredentials = false,
                        Credentials = new System.Net.NetworkCredential(senderemail.Address, password)
                    };
                    using (var mess = new MailMessage(senderemail, ps) { Subject = sub, Body = body })
                    {
                        smtp.Send(mess);
                    }
                }
            ...
            {
                //ViewBag.Error = "There are some problem sending email";
                return false;
            }
            return true;
        }
